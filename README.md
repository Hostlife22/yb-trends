...
- Operational API: `/api/v1/admin/sync-runs`, `/api/v1/admin/metrics`, `/api/v1/admin/alerts`
...
- SQLite storage (`trend_items`, `trend_points`, `sync_locks`, `sync_runs`) + schema migrations
...
- Quality Gate для sync (минимум записей + минимум доли релевантных)
- API integration/contract tests (`tests/test_api_integration.py`)
...
- CI smoke-step выполняет `python -m scripts.run_sync --region US --period 7d`
